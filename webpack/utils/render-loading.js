...
const extractAssetsInVueTpl = vueTplPath => {
  ...
  // 抽取template标签中的内容
  let html = /<div data-template = "loading">(.*)<\/div>/g.exec(vueTpl)[1]
  // 抽取style标签内的内容
...
let vueAssets = null
let vueTplPath = resolvePath('../../src/assets/spin/render-loading.html')
let gifPath = resolvePath('../../src/assets/spin/imgs/loading.gif')
let loading = {
  html: '',
  css: ''
}
